...
FeedbackRouter.route("/:block_id")
  // .all(requireAPIToken)
  .all(requireAuth)
  .all((req, res, next) => {
    FeedbackService.getBlockFeedbackById(req.app.get("db"), req.params.block_id)
      .then((feedback) => {
        if (!feedback) {
  ...
  .post(jsonParser, (req, res, next) => {
    ...
    const user_id = BlockService.getUserId(req.app.get("db"), user_name);
    ...
    FeedbackService.postFeedback(req.app.get("db"), newFeedback)
      .then((feedback) => {
        res
          .status(201)
